# TODO: Translation updated at 2017-05-02 20:02

# game/lorem1.rpy:36
translate russiantl lorem1_3d89afe3:

    # m "I went to the bar Lorem had suggested as our meeting place. When I looked around, I saw Bryce sitting at a table, talking to someone I couldn’t see. I lingered on him for a second, but soon spotted Lorem in the corner of my eye."
    m "Я пришёл в бар, который был предложен Лоремом как место нашей встречи. Осмотревшись, я заметил Брюса. Он сидел за столом и болтал с кем-то, кого я не смог разглядеть. Я на секунду остановил свой взгляд на нём, но вскоре заметил уголком глаза Лорема" #TODO

# game/lorem1.rpy:40
translate russiantl lorem1_fb5ec58c:

    # Lo "There you are!"
    Lo "Вот ты где!" #TODO

# game/lorem1.rpy:42
translate russiantl lorem1_e53ec95c:

    # c "Hey, Lorem."
    c "Привет, Лорем." #TODO

# game/lorem1.rpy:60
translate russiantl _call_skiptut_16_0ff5f677:

    # s "My records indicate you have already experienced this scene in a satisfactory manner. Would you like to skip to the end?"
    s "Мои записи зафиксировали, что вы уже прошли эту сцену удовлетворительным образом. Вы бы хотели пропустить её до конца?"

# game/lorem1.rpy:68
translate russiantl _call_skiptut_16_eee83226:

    # s "It looks like you've seen this before. Skip to the end of this scene?"
    s "Похоже, вы уже видели это раньше. Пропустить немного вперёд?"

# game/lorem1.rpy:76
translate russiantl _call_skiptut_16_e7709aa0:

    # s "So, it turns out you've seen this before. Either you could watch this again, or we could save some time and just skip to the end of this scene."
    s "Выходит, что вы уже проходили это раньше. Вы можете продолжить, или мы можем сэкономить немного времени и пройти до конца этой сцены."

# game/lorem1.rpy:91
translate russiantl _call_skiptut_16_0054b77b:

    # s "As you wish.{cps=2}..{/cps}{w=1.0}{nw}"
    s "Как вы пожелаете.{cps=2}..{/cps}{w=1.0}{nw}"

# game/lorem1.rpy:112
translate russiantl _call_skipcheck_16_0054b77b:

    # s "As you wish.{cps=2}..{/cps}{w=1.0}{nw}"
    s "Как вы пожелаете.{cps=2}..{/cps}{w=1.0}{nw}"

# game/lorem1.rpy:116
translate russiantl _call_skipcheck_16_165776d4:

    # Lo relieved "It's good to see you! I thought you might not show up."
    Lo relieved "Как я рад тебя видеть! Я думал, ты и не придёшь вовсе." #TODO

# game/lorem1.rpy:124
translate russiantl _call_skipcheck_16_c7d509db:

    # Lo normal "Not at all! I got here early."
    Lo normal "Вовсе нет! Я рано сюда пришёл." #TODO

# game/lorem1.rpy:130
translate russiantl _call_skipcheck_16_694b8196:

    # Lo happy "Yeah, I can certainly see that."
    Lo happy "Да, " #TODO TODO глянуть контекст

# game/lorem1.rpy:138
translate russiantl _call_skipcheck_16_13dd88b5:

    # Lo "I'm glad you came anyway."
    Lo "Я всё равно рад, что ты зашёл" #TODO

# game/lorem1.rpy:145
translate russiantl _call_skipcheck_16_2e0eeda6:

    # c "I'm kinda surprised you wanted to meet me here. Don't take this the wrong way, but are you even old enough to drink?"
    c "Я немного поражён твоим выбором места для встречи. Не пойми меня неправильно, но тебе вообще можно пить?" #TODO TODO возраст приплести

# game/lorem1.rpy:147
translate russiantl _call_skipcheck_16_d23a2b3d:

    # Lo think "What are you talking about?"
    Lo think "О чём ты?" #TODO

# game/lorem1.rpy:149
translate russiantl _call_skipcheck_16_cef70957:

    # Lo happy "Oh, I see."
    Lo happy "А-а, я понял." #TODO

# game/lorem1.rpy:151
translate russiantl _call_skipcheck_16_e802b109:

    # Lo normal "I can assure you I am a fully grown adult. My species doesn't get much bigger than this."
    Lo normal "Уверяю тебя, я уже совершеннолетний. Мой вид вырастает только до таких размеров." #TODO TODO fully grown adult | вид или род?

# game/lorem1.rpy:153
translate russiantl _call_skipcheck_16_649fddf0:

    # Lo happy "Speaking of which, are you fully grown yet? How big do humans usually get and how do you stack up, comparatively speaking?"
    Lo happy "Кстати говоря, а ты уже достаточно вырос? Насколько большими вырастают люди, и насколько ты вырос относительно их?" #TODO заковыристо

# game/lorem1.rpy:155
translate russiantl _call_skipcheck_16_dd38c1dc:

    # Lo think "Oh, and would you mind if I took some notes?"
    Lo think "А, и ещё: ты не будешь против, если я буду записывать?" #TODO

# game/lorem1.rpy:157
translate russiantl _call_skipcheck_16_45499fc8:

    # c "Before we get to that, you haven't even told me yet what exactly all this is for. Are you sure you're not a reporter?"
    c "Перед тем, как мы к этому перейдём, скажи, а для чего тебе всё это? Ты точно не журналист?" #TODO TODO пришлось перестроить, думаю, можно лучше | посмотреть, репортёр или журналист

# game/lorem1.rpy:159
translate russiantl _call_skipcheck_16_260d481d:

    # Lo happy "If I was, Sebastian wouldn’t have let me meet you in the first place."
    Lo happy "Если бы я им был, то Себастиан меня бы вообще к тебе не пустил." #TODO

# game/lorem1.rpy:161
translate russiantl _call_skipcheck_16_57ab5828:

    # c "And I thought the uniform was just a clever disguise."
    c "Я просто подумал, что твоя рабочая форма - просто хорошая маскировка." #TODO "хорошая" ли

# game/lorem1.rpy:163
translate russiantl _call_skipcheck_16_a750447a:

    # Lo normal "It's real, though. Sebastian knows I work for the local post office."
    Lo normal "Всё же, это не маскировка. Себастиан знает, что я работаю в местном почтовом офисе." #TODO или всё-таки отделении почты

# game/lorem1.rpy:165
translate russiantl _call_skipcheck_16_c4112abb:

    # c "Excuse me, but this still doesn't really tell me anything about why I'm here. You're a postman who wants to interview me for... what, exactly?"
    c "Прости, но я всё ещё не понимаю, зачем я здесь. Ты - почтальон, который хочет взять у меня интервью, но... зачем именно?" #TODO TODO

# game/lorem1.rpy:167
translate russiantl _call_skipcheck_16_4c2ac08e:

    # Lo relieved "Sorry, I guess I got ahead of myself here. Maybe a full introduction is in order, and after that, you can still decide whether you want to go through with this or not."
    Lo relieved "Прости, похоже, я слишком сильно зашёл вперёд. Наверное, стоит полностью представить себя, а потом ты решишь, хочешь ли ты быть опрошенным или нет." #TODO последняя часть.. meh

# game/lorem1.rpy:186
translate russiantl _call_skipcheck_16_6a88e8e8:

    # Lo happy "I recently graduated college with a degree in computing and moved in here with a good friend of mine."
    Lo happy "Недавно я выпустился из колледжа и получил специализацию, связанную с компьютерами, и переехал сюда со своим хорошим приятелем." #TODO TODO специализация...

# game/lorem1.rpy:188
translate russiantl _call_skipcheck_16_98593d8f:

    # Lo think "This town doesn't exactly offer the best opportunities to put my degree to good use, though."
    Lo think "Правда, этот городок не позволяет в полной мере пользоваться моей специализацией." #TODO

# game/lorem1.rpy:190
translate russiantl _call_skipcheck_16_f52dcfe5:

    # c "Then why move here in the first place?"
    c "Тогда зачем ты вообще сюда переехал?" #TODO

# game/lorem1.rpy:192
translate russiantl _call_skipcheck_16_d8bc587f:

    # Lo normal "Living in the city can be very expensive. I can always move later when I get a job there. I've got other plans right now, though."
    Lo normal "Жизнь в городе может обходиться дорого. Я всегда могу снова переехать, когда получу здесь работу. Но пока что у меня имеются другие планы." #TODO

# game/lorem1.rpy:194
translate russiantl _call_skipcheck_16_3ad10c5b:

    # Lo happy "I'm working on a video game."
    Lo happy "Я работаю над видеоигрой." #TODO

# game/lorem1.rpy:203
translate russiantl _call_skipcheck_16_b78abb33:

    # Lo normal "You got it!"
    Lo normal "" #TODO TODO КОНТЕКСТ

# game/lorem1.rpy:209
translate russiantl _call_skipcheck_16_2e662216:

    # Lo relieved "But this is serious!"
    Lo relieved "" #TODO TODO и ещё

# game/lorem1.rpy:217
translate russiantl _call_skipcheck_16_1db2bc1a:

    # Lo "Exactly!"
    Lo "" #TODO TODO и ещё

# game/lorem1.rpy:225
translate russiantl _call_skipcheck_16_2d0c2152:

    # Lo "Video games and computers are just starting to catch on in places like this town. Making good content will be very important to give a good first impression."
    Lo ""

# game/lorem1.rpy:227
translate russiantl _call_skipcheck_16_b7b18181:

    # Lo happy "And humans have always been part of our media - books, movies, and now games. With you on our side, this could be an amazing opportunity!"
    Lo happy ""

# game/lorem1.rpy:229
translate russiantl _call_skipcheck_16_ed1e847f:

    # c "Well..."
    c ""

# game/lorem1.rpy:231
translate russiantl _call_skipcheck_16_60287eb6:

    # Lo shy "Oh. Maybe this wasn't such a good idea after all."
    Lo shy ""

# game/lorem1.rpy:239
translate russiantl _call_skipcheck_16_e08aa85e:

    # Lo relieved "I don't know. You don't seem so thrilled about doing this anymore..."
    Lo relieved ""

# game/lorem1.rpy:249
translate russiantl _call_skipcheck_16_8daf20aa:

    # Lo "Besides, your job must be really stressful, and you're probably already in the spotlight all the time. And now, to top it all off, you get some postman harassing you for an interview."
    Lo ""

# game/lorem1.rpy:251
translate russiantl _call_skipcheck_16_2660554d:

    # Lo "If you wanted to leave, I'd get that."
    Lo ""

# game/lorem1.rpy:257
translate russiantl _call_skipcheck_16_dce6f68c:

    # c "Well, I'm already here and have nothing else to do, so let's make the most of this."
    c ""

# game/lorem1.rpy:259
translate russiantl _call_skipcheck_16_55eb3920:

    # Lo happy "Yes! Thank you!"
    Lo happy ""

# game/lorem1.rpy:265
translate russiantl _call_skipcheck_16_aded8efe:

    # c "Well, this was a waste of time."
    c ""

# game/lorem1.rpy:267
translate russiantl _call_skipcheck_16_87c9bd59:

    # Lo shy "Sorry. I won't bother you again."
    Lo shy ""

# game/lorem1.rpy:269
translate russiantl _call_skipcheck_16_21ec9701:

    # c "I hope so."
    c ""

# game/lorem1.rpy:271
translate russiantl _call_skipcheck_16_6e20b877:

    # m "Without another word, I got up from our table and left."
    m ""

# game/lorem1.rpy:299
translate russiantl _call_skipcheck_16_1b74c6ae:

    # c "At least I won't have to be so careful now. With a reporter, I'd have to worry about things being misinterpreted or being taken out of context."
    c ""

# game/lorem1.rpy:301
translate russiantl _call_skipcheck_16_8062b0e2:

    # Lo normal "You should still be careful. What you tell me now will influence how your entire species will be represented in my game."
    Lo normal ""

# game/lorem1.rpy:303
translate russiantl _call_skipcheck_16_5fef0849:

    # Lo happy "Don't worry, though. I'll treat this delicate matter with the required finesse."
    Lo happy ""

# game/lorem1.rpy:322
translate russiantl _call_skipcheck_16_fc346745:

    # Lo think "How should we do this? Let me think..."
    Lo think ""

# game/lorem1.rpy:324
translate russiantl _call_skipcheck_16_7e729438:

    # c "Why do you want to make this game in the first place?"
    c ""

# game/lorem1.rpy:326
translate russiantl _call_skipcheck_16_3097ad9e:

    # Lo happy "I've always wanted to make a video game. I've had this idea for a while, and it seems now is the right time to do it. It's almost as if you arrived at the perfect moment."
    Lo happy ""

# game/lorem1.rpy:328
translate russiantl _call_skipcheck_16_51a478e0:

    # c "I don't know. Don't you think my visit will overshadow your efforts? People may not be so interested in playing a game about humans anymore if they had a real one show up."
    c ""

# game/lorem1.rpy:330
translate russiantl _call_skipcheck_16_390e2221:

    # Lo normal "Good point, but the vast majority won't get to meet you personally, anyway. If I say I did and modeled the humans in the game after you, people will flock to it to get a similar experience."
    Lo normal ""

# game/lorem1.rpy:338
translate russiantl _call_skipcheck_16_adcb0d58:

    # Lo think "I guess you could say so."
    Lo think ""

# game/lorem1.rpy:344
translate russiantl _call_skipcheck_16_349dd55e:

    # Lo relieved "That's just a necessary evil, I think. They have to know somehow."
    Lo relieved ""

# game/lorem1.rpy:352
translate russiantl _call_skipcheck_16_140ef85b:

    # Lo happy "Awesome!"
    Lo happy ""

# game/lorem1.rpy:359
translate russiantl _call_skipcheck_16_2b1c8f4f:

    # c "Okay, but why focus on humans? What makes us so important?"
    c ""

# game/lorem1.rpy:361
translate russiantl _call_skipcheck_16_e5ba7d6f:

    # Lo think "If there’s one thing that people here love, it’s humans. It doesn’t matter if they just see them in the media or believe in them as mythical creatures."
    Lo think ""

# game/lorem1.rpy:363
translate russiantl _call_skipcheck_16_1526dff6:

    # Lo normal "For me, humans were always real. I just didn't know whether I'd ever get a chance to meet one myself. I guess I can cross that one off the bucket list."
    Lo normal ""

# game/lorem1.rpy:365
translate russiantl _call_skipcheck_16_242918ac:

    # Lo happy "Now that we're here, I can tell you that no expense will be spared to make our portrayal of humans as accurate as possible."
    Lo happy ""

# game/lorem1.rpy:383
translate russiantl _call_skipcheck_16_86be8d24:

    # Lo normal "For good reason! I just love humans."
    Lo normal ""

# game/lorem1.rpy:385
translate russiantl _call_skipcheck_16_3ee17091:

    # c "If you say it like that, it actually ends up sounding pretty weird."
    c ""

# game/lorem1.rpy:387
translate russiantl _call_skipcheck_16_1c08cd7f:

    # c "Besides, what if I turn out to be a horrible person? What would you think about humans then?"
    c ""

# game/lorem1.rpy:389
translate russiantl _call_skipcheck_16_9bf80b9c:

    # Lo think "It wouldn't change anything, because I wouldn't base my opinion of an entire species on interactions with a single individual."
    Lo think ""

# game/lorem1.rpy:391
translate russiantl _call_skipcheck_16_054e3918:

    # c "Then what is your current opinion of us based on?"
    c ""

# game/lorem1.rpy:393
translate russiantl _call_skipcheck_16_9420a168:

    # Lo normal "Just the myths we have and the previous portrayals of humans in our media, but I suppose that's why I'm here talking with you right now. I want the truth, and all of it!"
    Lo normal ""

# game/lorem1.rpy:395
translate russiantl _call_skipcheck_16_15f9c1a5:

    # c "Telling you all of it might not be such a good idea..."
    c ""

# game/lorem1.rpy:397
translate russiantl _call_skipcheck_16_ef14dde8:

    # Lo think "Oh yeah, I guess that would take a lot more time than we have today."
    Lo think ""

# game/lorem1.rpy:399
translate russiantl _call_skipcheck_16_620bc3d3:

    # c "Anyway, what's the actual game about? What do you do?"
    c ""

# game/lorem1.rpy:401
translate russiantl _call_skipcheck_16_33eb1364:

    # Lo happy "It's a community simulation game. It all starts with the player character moving into a procedurally generated community, which is inhabitated by all kinds of mythical creatures. Humans included."
    Lo happy ""

# game/lorem1.rpy:403
translate russiantl _call_skipcheck_16_fb9591f5:

    # c "Mythical creatures like humans. Mmm-hmmm."
    c ""

# game/lorem1.rpy:405
translate russiantl _call_skipcheck_16_b404187b:

    # Lo "Not only are the towns you move into unique, but the inhabitants are, too. There are a variety of traits that get randomized, both in looks and personality."
    Lo ""

# game/lorem1.rpy:407
translate russiantl _call_skipcheck_16_d879a92b:

    # Lo "There are many things you can do, but it's very open-ended and you can live your life there however you want to."
    Lo ""

# game/lorem1.rpy:415
translate russiantl _call_skipcheck_16_fafe2bd6:

    # Lo relieved "Well, I guess you're not part of the target audience, then."
    Lo relieved ""

# game/lorem1.rpy:423
translate russiantl _call_skipcheck_16_491774d2:

    # Lo normal "It is! At least, I hope others feel the same way about it."
    Lo normal ""

# game/lorem1.rpy:429
translate russiantl _call_skipcheck_16_c4bbdf67:

    # Lo think "Oh, really?"
    Lo think ""

# game/lorem1.rpy:431
translate russiantl _call_skipcheck_16_343057fe:

    # Lo happy "I couldn't ask for a better compliment than that!"
    Lo happy ""

# game/lorem1.rpy:436
translate russiantl _call_skipcheck_16_40ad1523:

    # c "What other mythical creatures are there besides humans?"
    c ""

# game/lorem1.rpy:438
translate russiantl _call_skipcheck_16_fca12b73:

    # Lo think "I haven’t worked that out yet, but humans alone give us a lot of material to work with."
    Lo think ""

# game/lorem1.rpy:440
translate russiantl _call_skipcheck_16_be8b7287:

    # c "How so?"
    c ""

# game/lorem1.rpy:442
translate russiantl _call_skipcheck_16_b613766c:

    # Lo normal "The question of the mythical human can be tackled in a variety of ways. There are many different interpretations of what humans are like."
    Lo normal ""

# game/lorem1.rpy:444
translate russiantl _call_skipcheck_16_56a7507d:

    # c "For example?"
    c ""

# game/lorem1.rpy:446
translate russiantl _call_skipcheck_16_2ad9e663:

    # Lo happy "In general, the mythical human can be divided into three categories: Firstly, the human as a physical creature; secondly, the human as a non-physical entity; and thirdly, the human as a spiritual being."
    Lo happy ""

# game/lorem1.rpy:448
translate russiantl _call_skipcheck_16_4facd386:

    # c "Let's start with the physical aspects."
    c ""

# game/lorem1.rpy:450
translate russiantl _call_skipcheck_16_12a75e24:

    # Lo normal "Sure. Since one of our myths tells us that the human who created us eventually turned into a dragon, there is a lot of room for interpretation as to how similar humans actually are to dragons."
    Lo normal ""

# game/lorem1.rpy:452
translate russiantl _call_skipcheck_16_c28061a0:

    # Lo think "I can see you don't have wings, but most of us are able to create fire in some way, or at least have a breath weapon. How about you?"
    Lo think ""

# game/lorem1.rpy:459
translate russiantl _call_skipcheck_16_d275ced1:

    # Lo "I don't think that counts."
    Lo ""

# game/lorem1.rpy:465
translate russiantl _call_skipcheck_16_4e5846ff:

    # Lo normal "Not really. Having hands that are dextrous enough to do that is remarkable, though."
    Lo normal ""

# game/lorem1.rpy:475
translate russiantl _call_skipcheck_16_a6d6377a:

    # Lo happy "Really? That's amazing!"
    Lo happy ""

# game/lorem1.rpy:477
translate russiantl _call_skipcheck_16_cf49fd2e:

    # c "I was just kidding."
    c ""

# game/lorem1.rpy:479
translate russiantl _call_skipcheck_16_8130a854:

    # Lo relieved "You shouldn't joke about things like that. This is serious."
    Lo relieved ""

# game/lorem1.rpy:481
translate russiantl _call_skipcheck_16_86148265:

    # c "Alright."
    c ""

# game/lorem1.rpy:488
translate russiantl _call_skipcheck_16_b35e3b39:

    # Lo "It is a bit strange, though, because our myths also say that you gave that ability to us."
    Lo ""

# game/lorem1.rpy:490
translate russiantl _call_skipcheck_16_c5aed8d5:

    # c "Interesting."
    c ""

# game/lorem1.rpy:492
translate russiantl _call_skipcheck_16_0cef7eef:

    # Lo happy "Maybe it’s meant to be taken literally – meaning that when that ability was given to us, you lost it."
    Lo happy ""

# game/lorem1.rpy:494
translate russiantl _call_skipcheck_16_8054cb2a:

    # c "Or maybe we never had it in the first place."
    c ""

# game/lorem1.rpy:496
translate russiantl _call_skipcheck_16_efda574e:

    # c "Humans are known to have created a lot of things out of nothing. After all, we only needed to discover how to create and use fire, because we didn't have a natural ability to do so."
    c ""

# game/lorem1.rpy:498
translate russiantl _call_skipcheck_16_177d8150:

    # c "We have a proverb that sums this up pretty well."
    c ""

# game/lorem1.rpy:500
translate russiantl _call_skipcheck_16_0bfb9c67:

    # Lo normal "What is it?"
    Lo normal ""

# game/lorem1.rpy:502
translate russiantl _call_skipcheck_16_4a5b7618:

    # c "Necessity is the mother of invention."
    c ""

# game/lorem1.rpy:504
translate russiantl _call_skipcheck_16_c3d29a7c:

    # Lo think "So a lack of natural abilities drove innovation forward. What an interesting thought."
    Lo think ""

# game/lorem1.rpy:506
translate russiantl _call_skipcheck_16_acf8adb4:

    # c "Here's another thing: You may have many different images of humans, but in the end, only one can be right."
    c ""

# game/lorem1.rpy:508
translate russiantl _call_skipcheck_16_6cfa7aeb:

    # c "It's also possible that what you've been led to believe about humans doesn't actually refer to us at all."
    c ""

# game/lorem1.rpy:510
translate russiantl _call_skipcheck_16_65122c50:

    # Lo relieved "Are you implying that you aren't human?"
    Lo relieved ""

# game/lorem1.rpy:512
translate russiantl _call_skipcheck_16_b7f880a3:

    # c "No, I'm just saying that if you really had been in contact with humans before as your myths imply, they would need to belong to the same species as me, and thus be very similar to myself."
    c ""

# game/lorem1.rpy:514
translate russiantl _call_skipcheck_16_5c00227f:

    # c "If they aren't, then either they weren't humans at all, or have very different origins."
    c ""

# game/lorem1.rpy:516
translate russiantl _call_skipcheck_16_96de6db3:

    # Lo think "So you say there is a possibility that our humans and your humans may be something different altogether?"
    Lo think ""

# game/lorem1.rpy:518
translate russiantl _call_skipcheck_16_637351f1:

    # Lo happy "I suppose you are right in the way that our images of humans are very inconsistent. They certainly can't all be humans in the way that we would refer to a single species. The name should probably be reserved for yours."
    Lo happy ""

# game/lorem1.rpy:520
translate russiantl _call_skipcheck_16_72d32989:

    # Lo normal "And we haven't even touched on the other two categories of humans."
    Lo normal ""

# game/lorem1.rpy:522
translate russiantl _call_skipcheck_16_9190026a:

    # c "What did you call them again?"
    c ""

# game/lorem1.rpy:524
translate russiantl _call_skipcheck_16_e48d651f:

    # Lo happy "Non-physical human entities and spiritual humans."
    Lo happy ""

# game/lorem1.rpy:526
translate russiantl _call_skipcheck_16_7f1295b3:

    # c "What's the difference?"
    c ""

# game/lorem1.rpy:528
translate russiantl _call_skipcheck_16_6bb06f89:

    # Lo think "Honestly, the definitions get a bit muddled here. Going by conventional belief, a ghost could be a human who has died and thus changed into a different form. We would call this a spiritual human."
    Lo think ""

# game/lorem1.rpy:530
translate russiantl _call_skipcheck_16_5b7b2cac:

    # Lo normal "An angel, on the other hand, would be classified as a non-physical entity."
    Lo normal ""

# game/lorem1.rpy:532
translate russiantl _call_skipcheck_16_e3d49b89:

    # c "What do you know about angels?"
    c ""

# game/lorem1.rpy:534
translate russiantl _call_skipcheck_16_19a36f81:

    # Lo happy "Angels basically look like humans with wings. Their existence in ancient scripts was used to lend some credence to the theories about humans having become dragons."
    Lo happy ""

# game/lorem1.rpy:536
translate russiantl _call_skipcheck_16_cafbb74d:

    # c "Because of the wings?"
    c ""

# game/lorem1.rpy:538
translate russiantl _call_skipcheck_16_93b4c2a4:

    # Lo "Exactly."
    Lo ""

# game/lorem1.rpy:540
translate russiantl _call_skipcheck_16_f66ed97f:

    # c "But some of your dragon species don't have wings, either."
    c ""

# game/lorem1.rpy:542
translate russiantl _call_skipcheck_16_e2063eb2:

    # Lo think "Yes. As I said, once you start delving into that kind of stuff, it all becomes very convoluted."
    Lo think ""

# game/lorem1.rpy:544
translate russiantl _call_skipcheck_16_b57cf12b:

    # c "When we met, you also told me something about a four-headed human."
    c ""

# game/lorem1.rpy:546
translate russiantl _call_skipcheck_16_cd5dac38:

    # Lo normal "Right. With that we'd go into the realm of creatures that just don't make much sense at all."
    Lo normal ""

# game/lorem1.rpy:548
translate russiantl _call_skipcheck_16_62c174e9:

    # Lo think "Having four heads just seems unlikely from an evolutionary perspective."
    Lo think ""

# game/lorem1.rpy:550
translate russiantl _call_skipcheck_16_5be894cd:

    # Lo "That might not really matter if we were talking about a non-physical entity or a spiritual being, though."
    Lo ""

# game/lorem1.rpy:552
translate russiantl _call_skipcheck_16_142a8591:

    # c "So in the end, you have a lot of different ideas whose only connection to each other is that you use the word human to describe them in some way."
    c ""

# game/lorem1.rpy:554
translate russiantl _call_skipcheck_16_a133a251:

    # Lo normal "And they share at least some basic characteristics. You fit those as well, by the way."
    Lo normal ""

# game/lorem1.rpy:556
translate russiantl _call_skipcheck_16_66b483cd:

    # c "What are they?"
    c ""

# game/lorem1.rpy:558
translate russiantl _call_skipcheck_16_223625a4:

    # Lo think "You certainly are not a reptile like us, but you don't look like a conventional mammal either."
    Lo think ""

# game/lorem1.rpy:560
translate russiantl _call_skipcheck_16_d21ff05e:

    # c "We are mammals, though."
    c ""

# game/lorem1.rpy:562
translate russiantl _call_skipcheck_16_1097c581:

    # Lo "Yes, but you are so different from other mammals we know that, ultimately, you are quite unique. We don't know any other sentient mammals."
    Lo ""

# game/lorem1.rpy:564
translate russiantl _call_skipcheck_16_adad4e4d:

    # c "You've got a point there, I'll give you that."
    c ""

# game/lorem1.rpy:566
translate russiantl _call_skipcheck_16_6dae6f38:

    # c "Strangely enough, we have plenty of human-like creatures in our mythologies as well."
    c ""

# game/lorem1.rpy:568
translate russiantl _call_skipcheck_16_3a0f9dbc:

    # Lo happy "Oh, do tell!"
    Lo happy ""

# game/lorem1.rpy:570
translate russiantl _call_skipcheck_16_84b644db:

    # c "You already mentioned ghosts, but some other ones include dwarves, giants, faeries and blemmyes."
    c ""

# game/lorem1.rpy:572
translate russiantl _call_skipcheck_16_99cc5a9c:

    # Lo normal "I think I've heard of most of these before, but what is a blemmyes?"
    Lo normal ""

# game/lorem1.rpy:574
translate russiantl _call_skipcheck_16_bc61390c:

    # c "Essentially, a blemmyes is a human without a head."
    c ""

# game/lorem1.rpy:576
translate russiantl _call_skipcheck_16_13a0d3a7:

    # Lo think "A human without a head? How does that work? How do they eat?"
    Lo think ""

# game/lorem1.rpy:578
translate russiantl _call_skipcheck_16_23045a97:

    # c "Instead of a head, they just have all their facial features on their chest."
    c ""

# game/lorem1.rpy:580
translate russiantl _call_skipcheck_16_c56b9fcb:

    # Lo relieved "That's a rather weird mental image."
    Lo relieved ""

# game/lorem1.rpy:582
translate russiantl _call_skipcheck_16_6ab78f6e:

    # c "There are also a lot of half-creatures that share some characteristics with humans and other animals."
    c ""

# game/lorem1.rpy:584
translate russiantl _call_skipcheck_16_abec4e3c:

    # Lo think "Like merhumans?"
    Lo think ""

# game/lorem1.rpy:586
translate russiantl _call_skipcheck_16_c48dc32f:

    # c "Sure."
    c ""

# game/lorem1.rpy:588
translate russiantl _call_skipcheck_16_3aea1e4e:

    # c "Lastly, there are creatures that are said to be able to shapeshift into human form."
    c ""

# game/lorem1.rpy:590
translate russiantl _call_skipcheck_16_e783f80f:

    # c "Strangely enough, some of our dragon myths said they could shapeshift into humans or that certain countries’ royalty are descended from dragons."
    c ""

# game/lorem1.rpy:592
translate russiantl _call_skipcheck_16_3ff38ba7:

    # Lo normal "That would imply that they are genetically compatible with humans."
    Lo normal ""

# game/lorem1.rpy:612
translate russiantl _call_skipcheck_16_abdd03bc:

    # Lo think "Sounds rather outlandish, if you ask me. We certainly can't shapeshift like that."
    Lo think ""

# game/lorem1.rpy:621
translate russiantl _call_skipcheck_16_6b37c29e:

    # Lo normal "Right. To be fair, we do have a lot of myths that seem quite out there as well compared to what you're like."
    Lo normal ""

# game/lorem1.rpy:627
translate russiantl _call_skipcheck_16_2752c6e4:

    # Lo happy "To be fair, I never tried. Hang on."
    Lo happy ""

# game/lorem1.rpy:629
translate russiantl _call_skipcheck_16_8ee9d226:

    # Lo think "..."
    Lo think ""

# game/lorem1.rpy:631
translate russiantl _call_skipcheck_16_a28401a3:

    # Lo normal "Did it work?"
    Lo normal ""

# game/lorem1.rpy:633
translate russiantl _call_skipcheck_16_020d6ce0:

    # c "Nope, still a dragon."
    c ""

# game/lorem1.rpy:635
translate russiantl _call_skipcheck_16_6a9873c0:

    # Lo sad "Awww."
    Lo sad ""

# game/lorem1.rpy:638
translate russiantl _call_skipcheck_16_ec4292d5:

    # Lo think "Anyway, how come you know so much about myths?"
    Lo think ""

# game/lorem1.rpy:640
translate russiantl _call_skipcheck_16_2a92c8b3:

    # c "All part of being an ambassador. Not only did I learn about dragon myths in order to compare them to what you're like, but I also studied myths about creatures that are similar to humans so I could find out how similar they were to yours."
    c ""

# game/lorem1.rpy:642
translate russiantl _call_skipcheck_16_0b33fa04:

    # Lo happy "Well, you're doing an excellent job so far. And it's bound to help me a lot with my game as well."
    Lo happy ""

# game/lorem1.rpy:644
translate russiantl _call_skipcheck_16_9d229f6a:

    # c "You're going to put all of that into the game?"
    c ""

# game/lorem1.rpy:646
translate russiantl _call_skipcheck_16_23818b09:

    # Lo normal "Let's just say it gives me some excellent material to work with."
    Lo normal ""

# game/lorem1.rpy:648
translate russiantl _call_skipcheck_16_433c0d98:

    # c "I thought you wanted to give an accurate portrayal of humans, not just collect myths."
    c ""

# game/lorem1.rpy:650
translate russiantl _call_skipcheck_16_baea8985:

    # Lo think "One species will certainly be modeled after you, but that doesn't mean we can't introduce some variety with the others."
    Lo think ""

# game/lorem1.rpy:652
translate russiantl _call_skipcheck_16_610f35b2:

    # Lo happy "People will love it if we also bring some of the more unusual myths to life."
    Lo happy ""

# game/lorem1.rpy:654
translate russiantl _call_skipcheck_16_89aefe22:

    # c "I see."
    c ""

# game/lorem1.rpy:656
translate russiantl _call_skipcheck_16_415eea89:

    # Lo normal "But since we also want each of the normal human characters to be unique, I'd like to know how much variety there really is within your species."
    Lo normal ""

# game/lorem1.rpy:658
translate russiantl _call_skipcheck_16_bea9746d:

    # Lo think "I've seen Reza before, so comparing him to you already gives me a bit of an idea - but how far do these differences go, exactly?"
    Lo think ""

# game/lorem1.rpy:660
translate russiantl _call_skipcheck_16_14145a58:

    # Lo "When we arrived here, you asked me if I was old enough to drink. That leads me to believe that your average human must be a lot taller."
    Lo ""

# game/lorem1.rpy:662
translate russiantl _call_skipcheck_16_fa74b8ac:

    # c "That's true. You could say Reza would be about average for an adult."
    c ""

# game/lorem1.rpy:664
translate russiantl _call_skipcheck_16_375c6cbb:

    # c "Not that it would be impossible to get a good deal bigger than him or even smaller than you, but those cases are rather rare and usually the result of medical conditions."
    c ""

# game/lorem1.rpy:666
translate russiantl _call_skipcheck_16_285740c1:

    # Lo normal "I see, I see."
    Lo normal ""

# game/lorem1.rpy:668
translate russiantl _call_skipcheck_16_2447aed2:

    # Lo "What about colors? Are there any humans who are blue like me?"
    Lo ""

# game/lorem1.rpy:670
translate russiantl _call_skipcheck_16_f72e6b17:

    # c "Not really. There is a certain spectrum of skin colors that can be lighter or darker, but so far I've seen a lot more variety in the dragons here."
    c ""

# game/lorem1.rpy:672
translate russiantl _call_skipcheck_16_602f23e2:

    # c "We do have quite a number of different eye colors, though."
    c ""

# game/lorem1.rpy:674
translate russiantl _call_skipcheck_16_6a49ecc6:

    # Lo think "Interesting."
    Lo think ""

# game/lorem1.rpy:676
translate russiantl _call_skipcheck_16_ae0c3ac6:

    # c "As for things that aren't immediately visible, we also have different blood types, but I assume you know about that."
    c ""

# game/lorem1.rpy:678
translate russiantl _call_skipcheck_16_7f71981e:

    # Lo normal "We certainly have those as well."
    Lo normal ""

# game/lorem1.rpy:680
translate russiantl _call_skipcheck_16_43482f04:

    # c "Some people even believe your blood type influences your personality."
    c ""

# game/lorem1.rpy:682
translate russiantl _call_skipcheck_16_fc46b979:

    # Lo think "That, on the other hand, is news to me."
    Lo think ""

# game/lorem1.rpy:684
translate russiantl _call_skipcheck_16_61e5afcb:

    # c "Anything else you want to know?"
    c ""

# game/lorem1.rpy:686
translate russiantl _call_skipcheck_16_845133b1:

    # Lo relieved "Phew, we already got so much. I'll have to think about if and how we can work all this into the game."
    Lo relieved ""

# game/lorem1.rpy:688
translate russiantl _call_skipcheck_16_b184e63b:

    # c "Have you worked on any other games before?"
    c ""

# game/lorem1.rpy:690
translate russiantl _call_skipcheck_16_de842598:

    # Lo normal "I worked on a few small projects during the course of my studies and also interned at a studio before. This is on a completely different level, though."
    Lo normal ""

# game/lorem1.rpy:692
translate russiantl _call_skipcheck_16_be8b7287_1:

    # c "How so?"
    c ""

# game/lorem1.rpy:694
translate russiantl _call_skipcheck_16_e26e90cd:

    # Lo happy "It's my game, so I'm the one calling the shots here. It's a very different thing than just making a few graphics when you're part of a bigger team, for sure."
    Lo happy ""

# game/lorem1.rpy:696
translate russiantl _call_skipcheck_16_86819a05:

    # c "I see. How long exactly have you been working on this?"
    c ""

# game/lorem1.rpy:698
translate russiantl _call_skipcheck_16_7a1ab47b:

    # Lo think "Phew. Must be several months now. I've got everything planned out and a lot of the groundwork has already been done."
    Lo think ""

# game/lorem1.rpy:700
translate russiantl _call_skipcheck_16_b055803d:

    # Lo normal "The most difficult things are the characters themselves, and to make sure all of the details are accurate."
    Lo normal ""

# game/lorem1.rpy:702
translate russiantl _call_skipcheck_16_4e8c7062:

    # c "Of course."
    c ""

# game/lorem1.rpy:704
translate russiantl _call_skipcheck_16_a51215b8:

    # c "Have you shown the game to other people yet?"
    c ""

# game/lorem1.rpy:706
translate russiantl _call_skipcheck_16_a371ebeb:

    # Lo happy "Sure! Feedback is a very important part of the development process, so I regularly gather ideas and opinions from others. I even made a website to do this!"
    Lo happy ""

# game/lorem1.rpy:708
translate russiantl _call_skipcheck_16_d21af984:

    # c "And what do they say?"
    c ""

# game/lorem1.rpy:710
translate russiantl _call_skipcheck_16_835230ec:

    # Lo think "All kinds of things, really. Sometimes it's hard to decide which ones have a good point and which can be disregarded."
    Lo think ""

# game/lorem1.rpy:712
translate russiantl _call_skipcheck_16_2afa4c62:

    # Lo normal "Overall, it seems there are many people who are looking forward to play my game, though."
    Lo normal ""

# game/lorem1.rpy:714
translate russiantl _call_skipcheck_16_d9d4691f:

    # c "How many are we talking about?"
    c ""

# game/lorem1.rpy:716
translate russiantl _call_skipcheck_16_5c0f4648:

    # Lo "I don't know, exactly. The counter on my website only tracks the number of visits, so I don’t actually know how many different people there are."
    Lo ""

# game/lorem1.rpy:718
translate russiantl _call_skipcheck_16_9b25687c:

    # c "What if it doesn't work out?"
    c ""

# game/lorem1.rpy:720
translate russiantl _call_skipcheck_16_346b54b1:

    # Lo think "What do you mean?"
    Lo think ""

# game/lorem1.rpy:722
translate russiantl _call_skipcheck_16_efc51e51:

    # c "Well, you're investing a certain amount of time and money into making this game. Ideally, you would make back your investment and hopefully get some profit on top of that."
    c ""

# game/lorem1.rpy:724
translate russiantl _call_skipcheck_16_869998d9:

    # Lo normal "If the goal is to make money, sure. However, I just wanted to try my hand at doing something like this. If some people are going to enjoy the end result, that's great."
    Lo normal ""

# game/lorem1.rpy:726
translate russiantl _call_skipcheck_16_1ed5e1ae:

    # Lo happy "Don't forget that I'll also have made something that I can put on my resume. At the very least, it'll have been a good learning experience."
    Lo happy ""

# game/lorem1.rpy:728
translate russiantl _call_skipcheck_16_ab1f35dd:

    # c "With that attitude, I suppose you really don't have anything to lose."
    c ""

# game/lorem1.rpy:730
translate russiantl _call_skipcheck_16_1c92ddc7:

    # Lo think "I can always look for another job, and maybe I'll end up working at a game studio, but I think once I've started doing something like that, I might not get this opportunity again."
    Lo think ""

# game/lorem1.rpy:732
translate russiantl _call_skipcheck_16_aa53918d:

    # c "So it's now or never, huh?"
    c ""

# game/lorem1.rpy:734
translate russiantl _call_skipcheck_16_9f523b59:

    # Lo normal "In a nutshell, yes."
    Lo normal ""

# game/lorem1.rpy:736
translate russiantl _call_skipcheck_16_99384fb3:

    # c "And that all works out with your courier job?"
    c ""

# game/lorem1.rpy:738
translate russiantl _call_skipcheck_16_11dcf77f:

    # Lo happy "Sure. I can support myself this way and am not really under any pressure, since I don't have any deadlines to keep."
    Lo happy ""

# game/lorem1.rpy:745
translate russiantl _call_skipcheck_16_823bd1ea:

    # Lo "I'd like to think so!"
    Lo ""

# game/lorem1.rpy:753
translate russiantl _call_skipcheck_16_ebb6432d:

    # Lo think "You're right about that. It's something where balance is important, I think. I set deadlines for myself, but I don't have to worry about the project breaking down if I don't meet certain milestones at a certain time."
    Lo think ""

# game/lorem1.rpy:756
translate russiantl _call_skipcheck_16_8455dd24:

    # c "What kind of tools are you using to make it? If I consider what some of your species look like, I'm having a hard time imagining how the bigger ones could program a game."
    c ""

# game/lorem1.rpy:758
translate russiantl _call_skipcheck_16_16ced629:

    # Lo normal "While we do have devices tailored to each individual species, I can't deny that my dextrous hands make it a lot easier compared to some of the others."
    Lo normal ""

# game/lorem1.rpy:760
translate russiantl _call_skipcheck_16_f7462fd2:

    # Lo "One downside is that I have to keep my nails short, or else using the keyboard gets a bit cumbersome."
    Lo ""

# game/lorem1.rpy:762
translate russiantl _call_skipcheck_16_df4298d3:

    # c "I imagine you'd have to be careful with those while working as a courier, anyway."
    c ""

# game/lorem1.rpy:764
translate russiantl _call_skipcheck_16_088e6966:

    # Lo relieved "You're right about that. One wrong move and you've suddenly shredded the letter you were just going to hand to someone."
    Lo relieved ""

# game/lorem1.rpy:779
translate russiantl _call_skipcheck_16_9041e3cd:

    # Lo normal "Luckily, that kind of thing doesn't really happen to me anymore."
    Lo normal ""

# game/lorem1.rpy:781
translate russiantl _call_skipcheck_16_b7ad971f:

    # Lo "Not only because of the shorter nails, but once you've worked there for a while, you just learn to be more careful."
    Lo ""

# game/lorem1.rpy:783
translate russiantl _call_skipcheck_16_57968227:

    # Lo think "It's a bit sad not to have them as a natural letter opener anymore, though."
    Lo think ""

# game/lorem1.rpy:785
translate russiantl _call_skipcheck_16_c538817e:

    # Lo normal "And it's not just for me. Some people ask me to open their letters for them, since doing that on their own can be difficult for some of the less dextrous and older ones."
    Lo normal ""

# game/lorem1.rpy:787
translate russiantl _call_skipcheck_16_5a56cb9b:

    # c "Have you been working as a courier for long?"
    c ""

# game/lorem1.rpy:789
translate russiantl _call_skipcheck_16_de586ebf:

    # Lo happy "I just got this job a few months ago, but I had a part-time summer job at the post office when I was growing up."
    Lo happy ""

# game/lorem1.rpy:791
translate russiantl _call_skipcheck_16_c9537556:

    # Lo normal "My species is perfectly suited for the job, so they were glad to have me on board."
    Lo normal ""

# game/lorem1.rpy:810
translate russiantl _call_skipcheck_16_1883cb8f:

    # Lo happy "Of course, being able to fly is a big plus. My size means I can't deliver any big packages, but I often get assigned to urgent letters that have to be delivered as soon as possible."
    Lo happy ""

# game/lorem1.rpy:812
translate russiantl _call_skipcheck_16_03042aef:

    # Lo think "Huh, it's getting late. Maybe we should leave it at that for today."
    Lo think ""

# game/lorem1.rpy:814
translate russiantl _call_skipcheck_16_a2b55cc8:

    # c "Yeah, I should probably head back to my apartment before it gets too dark."
    c ""

# game/lorem1.rpy:816
translate russiantl _call_skipcheck_16_260f4bd1:

    # Lo happy "I got more than enough material to work with. Thanks a lot!"
    Lo happy ""

# game/lorem1.rpy:821
translate russiantl _call_skipcheck_16_d4b333ad:

    # Lo think "Maybe I could convince you to meet me again some other time?"
    Lo think ""

# game/lorem1.rpy:823
translate russiantl _call_skipcheck_16_0fe8ac00:

    # Lo normal "I'd like to get some pictures of you for reference."
    Lo normal ""

# game/lorem1.rpy:827
translate russiantl _call_skipcheck_16_2631e6bd:

    # c "Alright, sure. Here's my number."
    c ""

# game/lorem1.rpy:829
translate russiantl _call_skipcheck_16_bdefd922:

    # Lo happy "I can't thank you enough for doing this. You've been a huge help!"
    Lo happy ""

# game/lorem1.rpy:831
translate russiantl _call_skipcheck_16_5101fbef:

    # c "Don't worry about it."
    c ""

# game/lorem1.rpy:833
translate russiantl _call_skipcheck_16_00237545:

    # c "I really have to get going now, so maybe I'll see you next time."
    c ""

# game/lorem1.rpy:835
translate russiantl _call_skipcheck_16_54d3c108:

    # Lo "Sure! Ciao!"
    Lo ""

# game/lorem1.rpy:837
translate russiantl _call_skipcheck_16_77d9b866:

    # c "Take care."
    c ""

# game/lorem1.rpy:869
translate russiantl lorem1skip_0816df93:

    # c "Don't worry about it. I'll just consider it a public service."
    c ""

# game/lorem1.rpy:871
translate russiantl lorem1skip_1847224a:

    # Lo normal "Honestly, I'm incredibly grateful for the opportunity, especially after nearly getting denied by Sebastian when I met you."
    Lo normal ""

# game/lorem1.rpy:873
translate russiantl lorem1skip_40b41860:

    # c "Just make the best of it."
    c ""

# game/lorem1.rpy:875
translate russiantl lorem1skip_937eba87:

    # Lo happy "I certainly will!"
    Lo happy ""

# game/lorem1.rpy:877
translate russiantl lorem1skip_aca24fe1:

    # c "I really have to get going now, so maybe I'll see you some other time."
    c ""

# game/lorem1.rpy:879
translate russiantl lorem1skip_edfe1555:

    # Lo normal "Sure! Have a good night."
    Lo normal ""

# game/lorem1.rpy:881
translate russiantl lorem1skip_77d9b866:

    # c "Take care."
    c ""

# game/lorem1.rpy:910
translate russiantl lorem1skip_0816df93_1:

    # c "Don't worry about it. I'll just consider it a public service."
    c ""

# game/lorem1.rpy:912
translate russiantl lorem1skip_5d6edec2:

    # Lo normal "Thank you for your time. Really."
    Lo normal ""

# game/lorem1.rpy:914
translate russiantl lorem1skip_3d1d58e7:

    # Lo sad "I-I promise I won't bother you again."
    Lo sad ""

# game/lorem1.rpy:916
translate russiantl lorem1skip_3070bc42:

    # c "What are you talking about?"
    c ""

# game/lorem1.rpy:918
translate russiantl lorem1skip_d098a468:

    # Lo relieved "Well, I can't help but feel like you still thought this was a waste of time."
    Lo relieved ""

# game/lorem1.rpy:920
translate russiantl lorem1skip_9ca6f404:

    # c "I have to admit, I was kinda annoyed about the whole thing. I guess it showed."
    c ""

# game/lorem1.rpy:922
translate russiantl lorem1skip_8d36ec37:

    # Lo shy "Don't get me wrong, I'm still incredibly grateful that you stuck around until the end. I'm just saying it won't happen again."
    Lo shy ""

# game/lorem1.rpy:924
translate russiantl lorem1skip_fe5f3366:

    # c "Okay."
    c ""

# game/lorem1.rpy:926
translate russiantl lorem1skip_7efc4cd0:

    # c "I really should be going now."
    c ""

# game/lorem1.rpy:928
translate russiantl lorem1skip_3ec77663:

    # Lo "Take care."
    Lo ""

# game/lorem1.rpy:930
translate russiantl lorem1skip_bf8cb88a:

    # c "Yeah, you too."
    c ""

translate russiantl strings:

    # lorem1.rpy:12
    old "Chapter 4 - Lorem 1"
    new ""

    # lorem1.rpy:16
    old "Chapter 3 - Lorem 1"
    new ""

    # lorem1.rpy:20
    old "Chapter 2 - Lorem 1"
    new ""

    # lorem1.rpy:24
    old "Chapter 1 - Lorem 1"
    new ""

    # lorem1.rpy:118
    old "Am I late?"
    new ""

    # lorem1.rpy:118
    old "Well, I'm here now."
    new ""

    # lorem1.rpy:118
    old "I almost didn't."
    new ""

    # lorem1.rpy:169
    old "Sounds good to me."
    new ""

    # lorem1.rpy:169
    old "I already regret coming here."
    new ""

    # lorem1.rpy:197
    old "That's what this is all about? A video game?"
    new ""

    # lorem1.rpy:197
    old "Oh, you said this was going to be about something serious."
    new ""

    # lorem1.rpy:197
    old "A video game? Sounds interesting."
    new ""

    # lorem1.rpy:233
    old "Why?"
    new ""

    # lorem1.rpy:233
    old "You're right about that. I can't help but feel a little deceived by this."
    new ""

    # lorem1.rpy:253
    old "Leave." #this refers to an action by the main character, and not an order to lorem
    new ""

    # lorem1.rpy:305
    old "Don't get ahead of yourself. We haven't even started yet."
    new ""

    # lorem1.rpy:305
    old "I'm sure of it."
    new ""

    # lorem1.rpy:305
    old "Okay."
    new ""

    # lorem1.rpy:332
    old "Does that mean I'm going to be your game's mascot?"
    new ""

    # lorem1.rpy:332
    old "So now I'm part of your viral marketing campaign, great."
    new ""

    # lorem1.rpy:332
    old "As long as it's based on the truth, you have my support."
    new ""

    # lorem1.rpy:367
    old "You certainly seem passionate enough about this."
    new ""

    # lorem1.rpy:367
    old "Those are some messed up priorities."
    new ""

    # lorem1.rpy:367
    old "I'm with you on this."
    new ""

    # lorem1.rpy:409
    old "Sounds boring."
    new ""

    # lorem1.rpy:409
    old "That sounds interesting."
    new ""

    # lorem1.rpy:409
    old "That sounds like my kind of game."
    new ""

    # lorem1.rpy:455
    old "I don't have a breath weapon, unless you count my morning breath."
    new ""

    # lorem1.rpy:455
    old "I can rub two sticks together to make fire. Does that count?"
    new ""

    # lorem1.rpy:455
    old "Of course. I can also move things with my mind."
    new ""

    # lorem1.rpy:595
    old "I guess."
    new ""

    # lorem1.rpy:595
    old "Apparently."
    new ""

    # lorem1.rpy:595
    old "Sounds gross."
    new ""

    # lorem1.rpy:615
    old "I suppose that's why they're just myths."
    new ""

    # lorem1.rpy:740
    old "Seems like you know what you're doing."
    new ""

    # lorem1.rpy:740
    old "Deadlines are a good way to ensure something gets done, though."
    new ""

    # lorem1.rpy:766
    old "That sounds unpleasant."
    new ""

    # lorem1.rpy:766
    old "You should be more careful."
    new ""

    # lorem1.rpy:793
    old "Why is that?"
    new ""

    # lorem1.rpy:793
    old "Oh, really?"
    new ""

